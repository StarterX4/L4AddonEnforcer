...
use md5::{Digest, Md5};
use std::{/* borrow::Cow, */ env, env::var_os, fmt::Debug, fs::{File, read_to_string, write, create_dir_all, copy, remove_file}, io::{BufReader, Read}, path::{Path, PathBuf}, process::exit};
...
fn main() -> std::io::Result<()> {
    ...
    if (name.is_empty() && !addon_file.is_empty()) || (!name.is_empty() && addon_file.is_empty()) {
        eprintln!("{} Both addon name and addon file path must be provided!", "Error:".red());
        exit(22);
    }
    // Validate addon name
    ...
    let addon_path = PathBuf::from(&addon_file);
        if var_os("DEBUG").is_some() {println!("{} {} {:?}",
													"[D]".blue(),
													"Addon path:".bold(),
													addon_path);}
    ...
    // Locate the Left 4 Dead 2 directory
        if var_os("DEBUG").is_some() || verbose {println!("{} Locating L4D2 directory...", "[D]".blue());}
    ...
    if !gameinfo_path.exists() {
        eprintln!("{} Unable to locate gameinfo.txt file. Is the game installation broken?", "Error:".red());
        exit(2);
    }

        if var_os("DEBUG").is_some() || verbose {println!("{} {} {:?}",
													"[D]".blue(),
    ...
    if !gameinfo_backup_path.exists() {
        if gameinfo_md5 != gameinfo_orig_md5 {
            println!("{} gameinfo.txt file seems to be modified!", "Warning:".yellow());
            ...
            println!("\ta {}'s bug you can report to the dev!", crate::env!("CARGO_PKG_NAME"));
        ...
        if var_os("DEBUG").is_some() || verbose {println!("{} Found a gameinfo.txt backup ({:?})",
													"[D]".blue(),
													gameinfo_backup_path.file_name().unwrap().to_string_lossy());}
    }

    // Create the new addon directory
        if var_os("DEBUG").is_some() || verbose {println!("{} Creating addon directory: {}", "[D]".blue(), name);}
    let addon_dir = l4d2_dir.join(format!("{}", name));
    let mut addon_dir_existed = false;
    if addon_dir.exists() {
        addon_dir_existed = true;
            if var_os("DEBUG").is_some() || verbose {println!("{} {} An addon directory with the same name already exists! Assuming this is an update.",
														"[D]".blue(),
														"Warning:".yellow());}
    }
    else { create_dir_all(&addon_dir)?; }

    // Copy the addon file to the new addon directory
        if var_os("DEBUG").is_some() || verbose {println!("{} Copying addon file to: {:?}",
                                                    "[D]".blue(),
                                                    addon_dir);}
    ...
    if destination.exists() {
        ...
        if var_os("DEBUG").is_some() || verbose {println!("{} Deleted {:?}",
                                                    "[D]".blue(),
                                                    destination);}
    }
    ...
    if lines.iter().any(|line| line.contains("Game") && line.contains(&name)) && addon_dir_existed {
        println!("Updated {} successfully.", name);
    }
    else {
        // Find the line with "Game update"
        let index = lines.iter().position(|line| line.contains("Game") && line.contains("update")).unwrap_or(0);
