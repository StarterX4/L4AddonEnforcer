...
fn main() -> std::io::Result<()> {
    ...
    if args.len() > 1 {
            ...
            match arg.as_str() {
                "-a" | "-f" | "--addon" | "--file" => {
                    if let Some(value) = iter.next() {
                        ...
                        eprintln!("Error: No addon.vpk file provided for {}", arg);
                        exit(22);
                    }
                },
                "-n" | "--name" => {
        ...
        println!("Error: No options were passed!");
        exit(22);
    }
...
//  }
    ...
    if !gameinfo_path.exists() {
        ...
        if var_os("DEBUG").is_some() {println!("Gameinfo.txt path: {:?}", gameinfo_path);}
    ...
    let gameinfo_backup_path = gameinfo_path.with_extension("txt.orig");
        if var_os("DEBUG").is_some() {println!("gameinfo_backup_path {:?}", gameinfo_backup_path);}
    if !gameinfo_backup_path.exists() {
        if gameinfo_md5 != gameinfo_orig_md5 {
            println!("Warning: gameinfo.txt file seems to be modified!");
            ...
            println!("\t\tYour gameinfo.txt MD5 hash is: {}", gameinfo_md5);
        }
        // Create a backup of the gameinfo.txt file
        copy(&gameinfo_path, &gameinfo_backup_path)?;
    }
    else {
        if var_os("DEBUG").is_some() {println!("Found a gameinfo.txt backup ({:?})", gameinfo_backup_path.file_name().unwrap().to_string_lossy());}
    }

    // Create the new addon directory
        if var_os("DEBUG").is_some() {println!("Creating addon directory: {}", name);}
